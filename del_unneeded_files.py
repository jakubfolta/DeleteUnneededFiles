...
for folder, subfolders, files in os.walk(dir_to_check):
    ...
    for file in files:
        ...
        if file_size >= file_size_to_del:
            folder_abspath = os.path.basename(folder)
            print('Size of "{}" file greater than 100MB.\nSize of file: {}\nFile: "{}" in folder "{}" send to trash.\n'\
            .format(file, file_size, file, folder_abspath))
    ...
    if total_size >= file_size_to_del and not subfolders:       # and size is bigger than 100 MB
        print('Size of files in "{}" folder greter than 100MB.'.format(folder_abspath))
        print('Size of folder: "{}"'.format(total_size))
        print('Folder: "{}" send to trash'.format(folder_abspath))
